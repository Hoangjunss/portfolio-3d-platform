...
Tiến độ: **18b → 27 XONG**, **30 XONG**, **40 / 41 / 42 / 43 XONG** (`education`, `event`, `wedding`, `fitness` đều xanh + build gate ✓). Backend **187/187**, frontend **106/106**, kit **48/48**. **Còn lại: 28, và 25 site (31–39, 44–59).** **Checklist 6 mục cho mọi site package:** chép config từ một site đã xanh, **không** từ `template-kit/` — xem `docs/reviews/2026-09-21-code-review-plan-41.md`. **Bẫy môi trường, đã gặp 3 lần — hai triệu chứng hỏng ĐỘC LẬP nhau, phải kiểm cả hai sau mỗi `npm install`:** (a) binary `@rolldown/binding-win32-x64-msvc/*.node` phải là 20796928 B; (b) `package-lock.json` không được thiếu `resolved` (wedding 206/233, fitness 222/280 — `npm ci` từ lockfile hỏng cài đúng con số không mà vẫn thoát 0). **`RecordTable` đã sửa hai lần** (cột Actions thành opt-in; bỏ ràng buộc index-signature) — nó vốn được thiết kế cho một ca dùng CRUD admin rồi bị liệt vào component dùng chung. **Quyết định cần ở tầng chương trình:** chuỗi UI hardcode tiếng Anh trong kit (`InquiryForm`, `SavedItemsPanel`, `RecordTable`: Search/Actions/Edit/Delete) — không site nào chữa được. **Chưa ai rà:** spec 2b + 63 plan file.
