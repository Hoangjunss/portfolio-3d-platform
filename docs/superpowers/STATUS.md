...
Tiến độ: **18b → 27 XONG**, **30 XONG**, **40 → 44 XONG** (`education`, `event`, `wedding`, `fitness`, `nonprofit` đều xanh + build gate ✓). Backend **187/187**, frontend **106/106**, kit **48/48**. **Còn lại: 28, và 24 site (31–39, 45–59).** **Checklist 6 mục cho mọi site package:** chép config từ một site đã xanh, **không** từ `template-kit/`. **Viết assertion có thu hẹp vùng** (`within(...)`) — ba site liên tiếp hỏng vì truy vấn toàn trang; đưa câu này vào handoff thì plan 44 xanh ngay lần đầu. **Bẫy install, 4/4 lần lockfile hỏng** (206/233, 222/280, 219/256 gói thiếu `resolved`; binary thì chỉ thỉnh thoảng cụt) — **sinh lại lockfile là bước bắt buộc**, đừng commit bản đầu. **Quyết định treo ở tầng chương trình:** chuỗi UI tiếng Anh hardcode trong kit (`InquiryForm`, `SavedItemsPanel`, `RecordTable`) cộng với chrome tiếng Anh ở mọi site `lang="vi"`. **Chưa ai rà:** spec 2b + 63 plan file.
