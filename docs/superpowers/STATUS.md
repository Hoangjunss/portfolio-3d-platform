...
Tiến độ: **18b → 27 XONG**, **30 XONG**, **40 / 41 / 42 XONG** (`education`, `event`, `wedding` đều xanh + build gate ✓). Backend **187/187**, frontend **106/106**, kit **47/47**. **Còn lại: 28, và 26 site (31–39, 43–59).** **Checklist 6 mục cho mọi site package:** chép config từ một site đã xanh, **không** từ `template-kit/` — xem `docs/reviews/2026-09-21-code-review-plan-41.md`. **Bẫy môi trường, đã gặp 2 lần:** `npm install` trong `templates/<site>/` có thể cho binary native cụt và lockfile thiếu `resolved` mà vẫn thoát 0 — kiểm kích thước binary (`@rolldown/binding-win32-x64-msvc/*.node` phải là 20796928 B) trước khi tin bất kỳ kết quả test nào. **Quyết định cần ở tầng chương trình:** 4 chuỗi UI hardcode tiếng Anh trong kit (`InquiryForm`: Message / Sent / Something went wrong; `SavedItemsPanel`: Remove) — không site nào chữa được. **Chưa ai rà:** spec 2b + 63 plan file.
