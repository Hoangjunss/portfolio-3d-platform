...
- [ ] **Step 3: Produce `templates/corporate/theme.ts`** exporting a `TemplateTheme`-conformant
...
- [ ] **Step 4: Produce `templates/corporate/app/globals.css`** defining the `--color-*`/
...
- [ ] **Step 7: Commit** — `design: Hallmark pass for corporate site theme`
...
- [ ] **Step 2: Run to verify it fails** — `cd templates/corporate && npx vitest run
  scripts/build-gate.test.mjs` (package/app don't exist yet, expect failure).
- [ ] **Step 3: `templates/corporate/package.json`**
...
- [ ] **Step 4: `templates/corporate/next.config.js`**
...
- [ ] **Step 5: `templates/corporate/tsconfig.json`** — same shape as `template-kit/tsconfig.json`
...
- [ ] **Step 7: `templates/corporate/data/seed.ts`** — the `ServiceItem`/`SERVICES` and
...
- [ ] **Step 8: `templates/corporate/app/layout.tsx`**
...
- [ ] **Step 9: `templates/corporate/app/page.tsx`** — imports `Hero`, `ItemGrid`, `PeopleGrid`,
...
- [ ] **Step 10: Run `cd templates/corporate && npm install && npm run build`, then `npx vitest run
...
- [ ] **Step 12: Commit** — `feat: scaffold corporate template Next.js app with static section composition`
...
- [ ] **Step 3: `templates/corporate/app/CallbackSection.tsx`**
...
- [ ] **Step 4: Wire `CallbackSection` into `page.tsx`** — replace Task 2 Step 9's marker with
  `<CallbackSection />`, positioned between `Hero` and `ItemGrid` per spec §2.
- [ ] **Step 5: Run `cd templates/corporate && npx vitest run`, verify all pass.**
...
- [ ] **Step 8: Commit** — `feat: add callback-request interactive feature to corporate site`
...
- [ ] **Step 9: Run the Hallmark `audit` capability** against the built site, score each criterion
  ...
  this plan complete.
- [ ] **Step 10: Commit** — `docs: plan 31 complete — corporate site design score`
