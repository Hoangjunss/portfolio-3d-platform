# Streetwear E-commerce Variant Site Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the Streetwear e-commerce variant demo site (`templates/shop-streetwear/`, slug
`shop-streetwear`, `display_order` 21 — interactive-demo spec §4.1 row #1) as an independent
Next.js static-export app consuming `@portfolio/template-kit`: run its own `hallmark` design pass
(high-contrast neutral + bold accent, condensed-sans direction), scaffold the app and compose
`Hero` → `PricedItemGrid`(products) → `Footer` with real seed data, and build a real cart feature
(`useLocalCollection` + `CartBadge`/`CartDrawer` + a mock checkout summary screen) with an
...
`templates/shop-streetwear/`-only component (interactive-demo spec §5's "what stays shared" rule).
This plan mirrors the baseline e-commerce site's plan
(`2026-09-20-34-ecommerce.md`) task-for-task — same architecture, same cart mechanism — swapping
only the catalog, theme direction, and `storageKey`.
...
**Spec:** `docs/superpowers/specs/2026-09-20-site-shop-streetwear-design.md` (this site's seed
data, theme brief, and cart data flow — delta against the baseline site's spec) plus
...
`CartBadge`, §4.1 this site's assigned mood family, §5 the per-site Hallmark convention, §7 the
per-site interaction-test gate, §8 the design scoring gate) and
`docs/superpowers/specs/2026-09-20-template-design-system-design.md` (§4 `Hero`/`PricedItemGrid`/
`Footer` behavior, §6 folder/thumbnail/export convention).
...
- **Hard prerequisite: plan 30 must be landed and its tests passing** before Task 2 can import
  ...
  components inside `templates/shop-streetwear/`.
...
- **No fabricated content.** Product names/prices in the seed data (spec §3.1) are the fictional
  ...
  spec §5).
- **Theming is directionally assigned, not fully fixed** — `templates/shop-streetwear/theme.ts`
  must satisfy `template-kit`'s `TemplateTheme` shape and pass `assertValidTheme` at build time;
  its actual values come only from Task 1's real Hallmark session, executing the "high-contrast
  neutral + bold accent, condensed-sans" direction assigned by master spec §4.1 — the direction is
  fixed, the exact hex/font/spacing values are not invented ahead of that session
...
- **`storageKey` must be `'shop-streetwear-cart'`, never `'ecommerce-cart'`** — spec §4.1: each
  site's cart is isolated in `localStorage`; reusing the baseline's key would leak state between
  sites for a visitor touring both in the same browser.
...
**Files:**
- Create: `templates/shop-streetwear/theme.ts`
- Create: `templates/shop-streetwear/app/globals.css`
...
- [ ] **Step 1: Invoke the `hallmark` skill, greenfield path**, with the industry/mood direction
  from `docs/superpowers/specs/2026-09-20-site-shop-streetwear-design.md` §5 as the starting
  brief: a streetwear/apparel storefront (fictional brand "BLKOUT Studio"), executing the
  **high-contrast neutral + bold accent, condensed-sans** direction assigned by master spec §4.1,
  with the 3 reference directions listed in that section as research starting points, not fixed
  commitments. Follow the skill's own research → direction → token-authoring process; do not
  shortcut to picking a font pair and hex values without the research step the skill defines.
- [ ] **Step 2: Execute the assigned direction, not re-litigate it** — unlike the baseline
  `ecommerce` site (which leaves cool-vs-warm as an open call), this site's mood family is fixed by
  master spec §4.1. The session's output must state the specific bold-accent hue chosen, the
  condensed-sans display font selected, and its body-font pairing, so the record exists for future
  criterion #10 comparisons against sibling sites (baseline `ecommerce`, `shop-homegoods`,
  `shop-electronics`).
- [ ] **Step 3: Author `templates/shop-streetwear/theme.ts`** exporting a `TemplateTheme`-conforming
...
- [ ] **Step 4: Author `templates/shop-streetwear/app/globals.css`** with the session's actual
  token values (`--color-*`, `--font-*`, `--space-*` custom properties matching `template-kit`'s
  CSS variable name contract, parent spec §5) plus this site's `theme.ts` mood applied — importing
  `template-kit`'s base tokens and overriding values per the session's picks, not redefining the
  variable *names*. The high-contrast base (near-black/near-white foundation, not a mid-grey "safe"
  palette) and the single bold accent color must both be real, deliberate token values, not a
  softened compromise between the two.
- [ ] **Step 5: Verify `assertValidTheme(themeFromThemeTs)` does not throw** — run a one-off
  ...
  imports `templates/shop-streetwear/theme.ts` and calls `assertValidTheme` on its default/named
  export, confirming no error.

### Acceptance criteria

- `templates/shop-streetwear/theme.ts` exists, exports a value satisfying `template-kit`'s
  `TemplateTheme` shape, and calling `assertValidTheme` on it does not throw.
- `templates/shop-streetwear/app/globals.css` exists and defines real values (not placeholders, not
  copied verbatim from another site) for every custom property `template-kit`'s components read.
- The Hallmark session's direction execution (bold-accent hue, condensed-sans display font, body
  font pairing — Step 2) is recorded — either inline as a short comment block at the top of
  `theme.ts` explaining the "why" (non-obvious per this plan's Global Constraints comment rule — a
  design decision is not "restating what code does") or in this plan file's own Task 1 notes once
  executed; either location is acceptable as long as a later reader can see the decision was made
...
- [ ] **Step 6: Commit** — `feat(shop-streetwear): Hallmark design pass — theme tokens`
...
**Files:**
- Create: `templates/shop-streetwear/app/layout.tsx`
- Create: `templates/shop-streetwear/app/page.tsx`
- Create: `templates/shop-streetwear/data/seed.ts`
- Create: `templates/shop-streetwear/public/thumbnail.webp` (placeholder — see Step 5 note)
- Create: `templates/shop-streetwear/next.config.js`
- Create: `templates/shop-streetwear/package.json`
- Test: `templates/shop-streetwear/scripts/build-gate.test.mjs`

**Interfaces:**
- Consumes: `Hero`, `PricedItemGrid`, `Footer` from `@portfolio/template-kit`'s barrel export (plan
  30 Task 5); `templates/shop-streetwear/theme.ts` / `app/globals.css` (Task 1).
- Produces: a building, statically-exportable page at `templates/shop-streetwear/` that Task 3's
  cart feature mounts inside, and the `public/thumbnail.webp` the portfolio platform's 3D carousel
  (main spec plan 12) resolves via the `templates` table row already seeded for
  `shop-streetwear` (plan 30 Task 1, `display_order` 21).

### Design decisions

**(a) `page.tsx` composes `Hero` → `PricedItemGrid` → `Footer` as a server component reading
directly from `templates/shop-streetwear/data/seed.ts`** — per this plan's Global Constraints
...
`/var/www/templates/shop-streetwear/`.
...
```javascript
// templates/shop-streetwear/scripts/build-gate.test.mjs
...
describe('shop-streetwear build gate', () => {
...
- [ ] **Step 2: Run to verify it fails** — `cd templates/shop-streetwear && npx vitest run
  scripts/build-gate.test.mjs` (directory/package don't exist yet, expect failure).
- [ ] **Step 3: `templates/shop-streetwear/data/seed.ts`** — the `PRODUCTS` array exactly as
  specified in `docs/superpowers/specs/2026-09-20-site-shop-streetwear-design.md` §3.1 (12
  `ProductItem` rows, `id`/`title`/`price` fields, no `image`), plus a small
  `STORE_NAME = 'BLKOUT Studio'` constant `Hero` and `layout.tsx` both read for headline/nav copy
  consistency.
- [ ] **Step 4: `templates/shop-streetwear/next.config.js`**
...
- [ ] **Step 5: `templates/shop-streetwear/public/thumbnail.webp`** — a minimal valid WEBP file
...
- [ ] **Step 6: `templates/shop-streetwear/package.json`**

```json
{
  "name": "shop-streetwear",
  "private": true,
  "version": "0.1.0",
  "scripts": {
...
- [ ] **Step 7: `templates/shop-streetwear/app/layout.tsx`** — root layout importing
  `app/globals.css` (Task 1), rendering a header containing the store name/nav and the cart client
  wrapper's `CartBadge` slot (Task 3 fills the actual `CartBadge` import in; this task renders the
  header structure and leaves the badge's mount point as Task 3's own client component boundary so
  `layout.tsx` itself stays a server component per Next.js App Router convention).
- [ ] **Step 8: `templates/shop-streetwear/app/page.tsx`** — composes `Hero` (headline/subhead/CTA
...
- [ ] **Step 9: Run the build-gate test, verify it passes** — `cd templates/shop-streetwear && npm
...
- [ ] **Step 11: Commit** — `feat(shop-streetwear): scaffold Next.js app, compose Hero/PricedItemGrid/Footer with seed data`
...
**Files:**
- Create: `templates/shop-streetwear/app/CartProvider.tsx`
- Create: `templates/shop-streetwear/app/CheckoutSummary.tsx`
- Create: `templates/shop-streetwear/app/CartProvider.test.tsx`

**Interfaces:**
- Consumes: `useLocalCollection`, `CartBadge`, `CartDrawer` from `@portfolio/template-kit`'s barrel
  export (plan 30 Tasks 4-5); `PRODUCTS` from `templates/shop-streetwear/data/seed.ts` (Task 2).
...
`useLocalCollection<CartLine>('shop-streetwear-cart', [])` call** — note the **different
`storageKey` from the baseline `ecommerce-cart`** (site spec §4.1) so this site's cart is isolated
from every other site's, including the baseline and the other 2 extra e-commerce variants — exposing
`CartBadge` and `CartDrawer` (with the add-or-update-qty logic from spec §4.3 wired into a handler
it exposes for `page.tsx`'s `PricedItemGrid.onSelect`) so there is exactly one hook instance for the
whole page — matching the hook's storageKey-isolation contract (plan 30 Task 4) and avoiding two
independent `useLocalCollection` calls silently diverging.
...
spec §4.3 (deferring to baseline spec §4.5's mechanism): computes subtotal from the current cart
lines, adds the fixed mock shipping fee (30,000 VND, same convention as the baseline site), shows a
total, and a "This is a demo — no order was placed" notice with a "Continue shopping" action that
closes the summary without mutating the cart.

- [ ] **Step 1: Write the failing interaction test**

```tsx
// templates/shop-streetwear/app/CartProvider.test.tsx
...
describe('shop-streetwear cart', () => {
  ...
  it('cart persists under its own storageKey, isolated from other sites', async () => {
    window.localStorage.setItem('ecommerce-cart', JSON.stringify([
      { id: 'prod-tumbler', title: 'Cội Nguồn Steel Tumbler 500ml', price: 189000, qty: 5 },
    ]));
    ...
    expect(JSON.parse(window.localStorage.getItem('ecommerce-cart')!)).toHaveLength(1);
  });
});
```

- [ ] **Step 2: Run to verify it fails** — module doesn't exist.
- [ ] **Step 3: `templates/shop-streetwear/app/CartProvider.tsx`**
- [ ] **Step 4: `templates/shop-streetwear/app/CheckoutSummary.tsx`**
...
- [ ] **Step 6: Run `cd templates/shop-streetwear && npx vitest run`, verify all pass.**
...
| M6 | hardcode `CartProvider`'s `useLocalCollection` call to `'ecommerce-cart'` instead of `'shop-streetwear-cart'` | the storageKey-isolation test |

- [ ] **Step 8: Commit** — `feat(shop-streetwear): add cart feature with mock checkout summary`
...
- [ ] **Step 9: Run the Hallmark `audit` capability against the built site**, score every criterion,
  ...
  number. Criterion #10 must explicitly weigh this site against the baseline `ecommerce` site and
  the other 2 extra e-commerce variants (`shop-homegoods`, `shop-electronics`), per site spec §5.
...
- **Spec coverage:** interactive-demo spec §3.1 (Task 3's `useLocalCollection` use), §3.2 row #4
  (Task 3's cart feature), §3.3 (Task 2/3's `CartDrawer`/`CartBadge` composition), §4.1 (this
  site's assigned mood family and its position among the 3 extra e-commerce variants), §5 (Task 1's
  Hallmark pass), §7 (Task 3's interaction test), §8 (Task 3's design-score table), §9 (this plan is
  "plan 51" in that section's execution order). Parent spec §4 (Task 2's `Hero`/`PricedItemGrid`/
  `Footer` composition, amended per this site's spec §2 for the real `onSelect`), §6 (Task 2's
  folder/thumbnail/export convention). Site spec `2026-09-20-site-shop-streetwear-design.md` §3
  (Task 2's seed data), §4 (Task 3's cart/checkout data flow delta against the baseline), §5
  (Task 1's design brief).
- **Explicitly not built here:** any change to `template-kit` itself (a genuinely missing kit
  ...
  the baseline `ecommerce` site (already landed, plan 34) and the other 2 extra e-commerce variants
  (`shop-homegoods`, `shop-electronics`, interactive-demo spec §4.1 — separate later plans); real
  thumbnail upload through the admin media library (parent spec §6, an admin action); CI wiring for
  this site's own static-export deploy (plan 17, a parameter change to an existing pipeline, out of
  scope here per interactive-demo spec §10).
...
- **Type consistency checked:** `CartProvider`'s `CartLine` usage (Task 3) is exactly
  ...
  bespoke extension. `storageKey` is `'shop-streetwear-cart'`, distinct from the baseline's
  `'ecommerce-cart'` — verified by Task 3's dedicated isolation test.
- **Next step:** once this plan's design score (Task 3) clears the 9/10 bar on every criterion, this
  plan is complete. `shop-homegoods` and `shop-electronics` (interactive-demo spec §4.1) can then
  reuse this plan's and plan 34's Task 2/3 structure nearly verbatim, swapping only the seed
  catalog, the Hallmark brief's mood family, and the `storageKey`.
