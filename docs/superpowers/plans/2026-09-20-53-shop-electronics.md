# Shop-Electronics E-commerce Variant Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the consumer-electronics E-commerce variant demo site (`templates/shop-electronics/`,
slug `shop-electronics`, `display_order` 23 — interactive-demo spec §4.1) as an independent Next.js
static-export app consuming `@portfolio/template-kit`: run its own `hallmark` design pass (cool
anchor hue / grotesque-sans direction), scaffold the app and compose `Hero` → `PricedItemGrid`
...
`templates/shop-electronics/`-only component (interactive-demo spec §5's "what stays shared" rule).
This plan mirrors plan 34's (baseline `ecommerce`) task structure and design decisions nearly
verbatim, per that plan's own "Next step" note — only the catalog, theme direction, and
`storageKey` differ.
...
**Spec:** `docs/superpowers/specs/2026-09-20-site-shop-electronics-design.md` (this site's seed
data, theme brief, and cart data flow) plus
`docs/superpowers/specs/2026-09-20-interactive-demo-templates-design.md` (§3.1 `useLocalCollection`
contract, §3.3 `CartDrawer`/`CartBadge`, §4.1 the 3 extra e-commerce variants and this site's cool-hue/
grotesque-sans direction, §5 the per-site Hallmark convention, §7 the per-site interaction-test gate,
§8 the design scoring gate) and `docs/superpowers/specs/2026-09-20-template-design-system-design.md`
(§4 `Hero`/`PricedItemGrid`/`Footer` behavior, §6 folder/thumbnail/export convention).
...
- **Hard prerequisite: plan 30 must be landed and its tests passing** before Task 2 can import
  ...
  components inside `templates/shop-electronics/`.
...
- **No fabricated content.** Product names/prices in the seed data (spec §3.1) are the fictional
  ...
  spec §5).
- **Theming direction is a non-binding starting point, not a fixed cluster** —
  `templates/shop-electronics/theme.ts` must satisfy `template-kit`'s `TemplateTheme` shape and pass
  `assertValidTheme` at build time; its actual values come only from Task 1's real Hallmark session
  (research → direction → tokens), not invented ahead of that session, even though the session
  starts from the cool-hue/grotesque-sans direction the master spec assigns this site
  (interactive-demo spec §4.1, site spec §5).
...
- Current baseline per `docs/superpowers/STATUS.md`: **re-read `STATUS.md`'s own header for the
  actual current test-count baseline before reporting** — plan 30, plan 34, and any sites
  authored/landed before this one may have changed it.
...
**Files:**
- Create: `templates/shop-electronics/theme.ts`
- Create: `templates/shop-electronics/app/globals.css`
...
- [ ] **Step 1: Invoke the `hallmark` skill, greenfield path**, with the industry/mood direction
  from `docs/superpowers/specs/2026-09-20-site-shop-electronics-design.md` §5 as the starting
  brief: a consumer-electronics specialty storefront (fictional brand "Vòng Cung Digital"), cool
  anchor hue / grotesque-sans display as the *starting* direction (interactive-demo spec §4.1 —
  unlike the baseline `ecommerce` site, this site's hue family is not an open call, but the exact
  hue value, type pairing, spacing scale, and motion are still real session output, not fixed here),
  with the 3 reference directions listed in that spec section as research starting points, not fixed
...
- [ ] **Step 2: Confirm distinctiveness against sibling sites** — the session's output must state
  how this site's direction differs concretely (not just "different hex value") from the baseline
  `ecommerce` site (plan 34, general/neutral retail), `shop-homegoods` (warm/serif), and
  `shop-streetwear` (high-contrast/bold/condensed), plus any other cool-hue sites already landed at
  the time this plan executes (e.g. `corporate`, `saas`), so the record exists for master spec §8
  criterion #10 self-scoring later in Task 3.
- [ ] **Step 3: Author `templates/shop-electronics/theme.ts`** exporting a `TemplateTheme`-conforming
...
- [ ] **Step 4: Author `templates/shop-electronics/app/globals.css`** with the session's actual
  token values (`--color-*`, `--font-*`, `--space-*` custom properties matching `template-kit`'s CSS
...
- [ ] **Step 5: Verify `assertValidTheme(themeFromThemeTs)` does not throw** — run a one-off
  ...
  imports `templates/shop-electronics/theme.ts` and calls `assertValidTheme` on its default/named
  export, confirming no error.

### Acceptance criteria

- `templates/shop-electronics/theme.ts` exists, exports a value satisfying `template-kit`'s
  `TemplateTheme` shape, and calling `assertValidTheme` on it does not throw.
- `templates/shop-electronics/app/globals.css` exists and defines real values (not placeholders, not
  copied verbatim from another site, including the baseline `ecommerce` or the other 2 extra
  variants) for every custom property `template-kit`'s components read.
- The Hallmark session's direction and distinctiveness rationale (Step 2) are recorded — either
  inline as a short comment block at the top of `theme.ts` explaining the "why" (non-obvious per
  this plan's Global Constraints comment rule — a design decision is not "restating what code does")
  or in this plan file's own Task 1 notes once executed; either location is acceptable as long as a
  later reader can see the decision was made deliberately, not defaulted.
...
- [ ] **Step 6: Commit** — `feat(shop-electronics): Hallmark design pass — theme tokens`
...
**Files:**
- Create: `templates/shop-electronics/app/layout.tsx`
- Create: `templates/shop-electronics/app/page.tsx`
- Create: `templates/shop-electronics/data/seed.ts`
- Create: `templates/shop-electronics/public/thumbnail.webp` (placeholder — see Step 5 note)
- Create: `templates/shop-electronics/next.config.js`
- Create: `templates/shop-electronics/package.json`
- Test: `templates/shop-electronics/scripts/build-gate.test.mjs`

**Interfaces:**
- Consumes: `Hero`, `PricedItemGrid`, `Footer` from `@portfolio/template-kit`'s barrel export (plan
  30 Task 5); `templates/shop-electronics/theme.ts` / `app/globals.css` (Task 1).
- Produces: a building, statically-exportable page at `templates/shop-electronics/` that Task 3's
  cart feature mounts inside, and the `public/thumbnail.webp` the portfolio platform's 3D carousel
  (main spec plan 12) resolves via the `templates` table row already seeded for `shop-electronics`
  (plan 30 Task 1, `display_order` 23).

### Design decisions

**(a) `page.tsx` composes `Hero` → `PricedItemGrid` → `Footer` as a server component reading
directly from `templates/shop-electronics/data/seed.ts`** — per this plan's Global Constraints
...
`/var/www/templates/shop-electronics/`.
...
```javascript
// templates/shop-electronics/scripts/build-gate.test.mjs
...
describe('shop-electronics build gate', () => {
...
- [ ] **Step 2: Run to verify it fails** — `cd templates/shop-electronics && npx vitest run
  scripts/build-gate.test.mjs` (directory/package don't exist yet, expect failure).
- [ ] **Step 3: `templates/shop-electronics/data/seed.ts`** — the `PRODUCTS` array exactly as
  specified in `docs/superpowers/specs/2026-09-20-site-shop-electronics-design.md` §3.1 (12
  `ProductItem` rows, `id`/`title`/`price` fields, no `image`), plus a small
  `STORE_NAME = 'Vòng Cung Digital'` constant `Hero` and `layout.tsx` both read for headline/nav
  copy consistency.
- [ ] **Step 4: `templates/shop-electronics/next.config.js`**
...
- [ ] **Step 5: `templates/shop-electronics/public/thumbnail.webp`** — a minimal valid WEBP file
...
- [ ] **Step 6: `templates/shop-electronics/package.json`**
...
- [ ] **Step 7: `templates/shop-electronics/app/layout.tsx`** — root layout importing
...
- [ ] **Step 8: `templates/shop-electronics/app/page.tsx`** — composes `Hero` (headline/subhead/CTA
  ...
  wrapper Task 3 provides) → `Footer` (link set: Shop, Support, Warranty & Returns, Contact;
...
- [ ] **Step 9: Run the build-gate test, verify it passes** — `cd templates/shop-electronics && npm
...
- [ ] **Step 11: Commit** — `feat(shop-electronics): scaffold Next.js app, compose Hero/PricedItemGrid/Footer with seed data`
...
**Files:**
- Create: `templates/shop-electronics/app/CartProvider.tsx`
- Create: `templates/shop-electronics/app/CheckoutSummary.tsx`
- Create: `templates/shop-electronics/app/CartProvider.test.tsx`

**Interfaces:**
- Consumes: `useLocalCollection`, `CartBadge`, `CartDrawer` from `@portfolio/template-kit`'s barrel
  export (plan 30 Tasks 4-5); `PRODUCTS` from `templates/shop-electronics/data/seed.ts` (Task 2).
...
`useLocalCollection<CartLine>('shop-electronics-cart', [])` call** — note the storage key is
`shop-electronics-cart`, distinct from the baseline `ecommerce-cart` and the other 2 extra
variants' own keys, so each site's cart is fully isolated per the hook's storageKey-isolation
contract (plan 30 Task 4) — exposing `CartBadge` and `CartDrawer` (with the add-or-update-qty logic
from spec §4 wired into a handler it exposes for `page.tsx`'s `PricedItemGrid.onSelect`) so there is
exactly one hook instance for the whole page, avoiding two independent `useLocalCollection` calls
silently diverging.
...
spec §4.5 (reusing the baseline site's §4.5 narrative): computes subtotal from the current cart
lines, adds the fixed mock shipping fee (30,000 VND), shows a total, and a "This is a demo — no
...
```tsx
// templates/shop-electronics/app/CartProvider.test.tsx
...
describe('shop-electronics cart', () => {
...
- [ ] **Step 3: `templates/shop-electronics/app/CartProvider.tsx`**
- [ ] **Step 4: `templates/shop-electronics/app/CheckoutSummary.tsx`**
...
- [ ] **Step 6: Run `cd templates/shop-electronics && npx vitest run`, verify all pass.**
...
- [ ] **Step 8: Commit** — `feat(shop-electronics): add cart feature with mock checkout summary`
...
- **Spec coverage:** interactive-demo spec §3.1 (Task 3's `useLocalCollection` use), §3.3 (Task 2/3's
  `CartDrawer`/`CartBadge` composition), §4.1 (this site's slug/industry/mood assignment), §5 (Task
  1's Hallmark pass), §7 (Task 3's interaction test), §8 (Task 3's design-score table), §9 (this
  plan is "plan 53" in that section's execution order). Parent spec §4 (Task 2's
  `Hero`/`PricedItemGrid`/`Footer` composition, amended per this site's spec §2 for the real
  `onSelect`), §6 (Task 2's folder/thumbnail/export convention). Site spec
  `2026-09-20-site-shop-electronics-design.md` §3 (Task 2's seed data), §4 (Task 3's full
  cart/checkout data flow, reusing the baseline `ecommerce` spec's §4 narrative), §5 (Task 1's
  design brief).
- **Explicitly not built here:** any change to `template-kit` itself (a genuinely missing kit
  ...
  the other 2 extra e-commerce variants (`shop-streetwear`/`shop-homegoods`, interactive-demo spec
  §4.1 — separate plans); real thumbnail upload through the admin media library (parent spec §6, an
  admin action); CI wiring for this site's own static-export deploy (plan 17, a parameter change to
  an existing pipeline, out of scope here per interactive-demo spec §10).
...
- **Type consistency checked:** `CartProvider`'s `CartLine` usage (Task 3) is exactly
  ...
  bespoke extension. `storageKey` is `shop-electronics-cart`, distinct from `ecommerce-cart` (plan
  34) and from the other 2 extra variants' own keys.
- **Structural precedent:** this plan mirrors plan 34 (baseline `ecommerce`)'s exact task structure
  and mutation-check set nearly verbatim, per that plan's own "Next step" note inviting the 3 extra
  variant plans to reuse Task 2/3 structure — only the catalog (site spec §3.1), the Hallmark
  direction (site spec §5, cool-hue/grotesque-sans vs. the baseline's open hue call), and the
  `storageKey` differ.
- **Next step:** once this plan's design score (Task 3) clears the 9/10 bar on every criterion, this
  plan is complete. The remaining 2 extra e-commerce variant plans (`shop-streetwear`,
  `shop-homegoods`) can reuse this plan's Task 2/3 structure the same way this plan reused plan 34's.
